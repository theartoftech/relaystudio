...
def add_cover(doc: Document, title: str, subtitle: str, audience: str) -> None:
    ...
    p = doc.add_paragraph("Version 1.0 | Sprint 15 | July 2026")
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p.runs[0].font.color.rgb = RGBColor.from_string(MUTED)
    doc.add_page_break()
...
def render_preview(spec: DiagramSpec) -> Path:
    ...
    draw.text((60, height - 55), "Relay Studio | Sprint 15 documentation", font=font(16), fill="#5F6B7A")
...
def onboarding_sections() -> tuple[SectionSpec, ...]:
    return (
        ...
        SectionSpec("OpenAPI Import Failures", ("The importer accepts a Swagger UI page or direct Swagger/OpenAPI document, validates supported formats, previews operations, and changes the project only after explicit selection."), bullets=("Separate URL discovery failures from definition parsing failures.", "Verify relative definition URLs against the page URL.", "Check Swagger 2.0 versus OpenAPI 3.x server/base-path construction.", "Confirm unsupported methods or schemas produce actionable errors.", "Confirm examples do not import bearer tokens, passwords, API keys, cookies, or credentials.", "Test Select All, Clear, partial selection, cancellation, and duplicate ID generation.")),
...
def architecture_sections() -> tuple[SectionSpec, ...]:
    return (
        ...
        SectionSpec("Product Capabilities", bullets=("Create, rename, delete, save, reopen, import, and recover local projects.", "Design REST services with method, URL, parameters, headers, body, auth, timeout, retry, proxy, and environment variables.", "Import a Swagger UI page or direct OpenAPI/Swagger definition and explicitly select operations.", "Run and cancel individual requests, inspect formatted responses, and export redacted diagnostics.", "Compose visual flows with dependency edges, failure branches, variable mappings, cleanup, and cancellation.", "Save redacted response artifacts and reopen them for debugging.", "Package unsigned developer betas for macOS, Windows, and Linux.")),
        ...
        SectionSpec("OpenAPI Import", ("The importer treats both the supplied URL and document as untrusted. It discovers definition URLs from Swagger UI pages, supports Swagger 2.0 and OpenAPI 3.x documents, previews operations, and converts only explicitly selected services."), bullets=("The project is unchanged until confirmation.", "Imported service IDs are collision-safe.", "Relative servers and paths resolve against the source.", "Credentials and credential-like examples are never imported.", "External references, multipart bodies, and additional methods are deferred to Sprint 16.")),
...
def sprint_sections() -> tuple[SectionSpec, ...]:
    ...
    sprint_summaries = (
        ...
        ("Sprint 16 (planned)", "Developer productivity and import hardening: external references, form/multipart bodies, more methods, response comparison, and beta-driven fixes."),
    )
...
def test_sections() -> tuple[SectionSpec, ...]:
    return (
        ...
        SectionSpec("Selective OpenAPI Import", steps=("Choose File > Import API Definition.", "Enter a direct OpenAPI/Swagger URL and a Swagger UI page URL in separate cases.", "Verify definition discovery and preview metadata.", "Use Select All and Clear, then choose a strict subset of operations.", "Import and confirm only selected services are added.", "Inspect generated method, path, parameters, body examples, folders, auth placeholders, IDs, dirty state, and source metadata.", "Repeat invalid URL, invalid document, unsupported version, zero-selection, duplicate-operation, and credential-like example cases.")),
...
def security_sections() -> tuple[SectionSpec, ...]:
    return (
        ...
        SectionSpec("Known Risks and Deferrals", bullets=("Unsigned packages can trigger platform trust warnings.", "External OpenAPI references and multipart/form imports remain deferred.", "User-configured endpoints may return hostile or very large data; response limits and formatting must remain bounded.", "Local project files are only as protected as the user's filesystem account.", "There is no hosted collaboration, public updater, store distribution, or enterprise deployment certification.")),
...
def main() -> None:
    ...
    build_manual(
        ...
        onboarding_sections(),
        "Read sequentially during onboarding, then return to the subsystem playbooks while debugging. Commands and paths are verified against the repository at Sprint 15.",
    )
    build_manual(
