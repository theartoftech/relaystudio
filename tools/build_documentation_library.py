...
def security_sections() -> tuple[SectionSpec, ...]:
    return (
        ...
        SectionSpec("Known Risks and Deferrals", bullets=("Unsigned packages can trigger platform trust warnings.", "External OpenAPI references are intentionally same-origin and bounded; cross-origin graphs require the API owner to consolidate or proxy definitions locally.", "Multipart file paths are persisted locally and file contents are read only at native send time; each file is limited to 25 MiB. Raw application/octet-stream bodies, streaming uploads, and browser file sends remain deferred.", "User-configured endpoints may return hostile or very large data; response limits and formatting must remain bounded.", "Local project files are only as protected as the user's filesystem account.", "There is no hosted collaboration, public updater, store distribution, or enterprise deployment certification.")),
...
def main() -> None:
    ...
    build_manual(
        ...
        onboarding_sections(),
        "Read sequentially during onboarding, then return to the subsystem playbooks while debugging. Commands and paths are verified against the repository at Sprint 17.",
    )
    build_manual(
