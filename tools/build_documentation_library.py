...
def add_cover(doc: Document, title: str, subtitle: str, audience: str) -> None:
    ...
    p = doc.add_paragraph("Version 1.1 | Sprint 16 | July 2026")
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p.runs[0].font.color.rgb = RGBColor.from_string(MUTED)
    doc.add_page_break()
...
def render_preview(spec: DiagramSpec) -> Path:
    ...
    draw.text((60, height - 55), "Relay Studio | Sprint 16 documentation", font=font(16), fill="#5F6B7A")
...
def onboarding_sections() -> tuple[SectionSpec, ...]:
    return (
        ...
        SectionSpec("Debugging Requests and Flows", ("Request execution is a typed pipeline: resolve variables, validate, build, transport, format, redact diagnostics, and update visible state. Flows add dependency ordering, mappings, branching, cleanup, cancellation, and node status."), steps=("Reproduce with the smallest request and a controlled local endpoint.", "Check resolved URL, headers, query/path parameters, auth mode, timeout, retry policy, and active environment without printing secret values.", "Classify the failure as validation, transport, HTTP response, parsing, mapping, cancellation, or persistence.", "Inspect the visible console and exported diagnostics for a matching typed error and redacted values.", "For a flow, inspect the first non-success node and verify downstream blocked/skipped behavior.")),
        SectionSpec("Debugging Native Tauri Behavior", ("A browser success does not prove native behavior. File dialogs, atomic writes, menu commands, packaged resources, proxy use, and native HTTP require a Tauri run."), bullets=("Read the Rust terminal output and browser/webview console together.", "Confirm the command is registered, included in the reviewed capability, and invoked with the expected payload.", "Test permission denial, missing files, malformed project data, and interrupted writes.", "Never broaden filesystem or network capabilities to hide a specific error.")),
        SectionSpec("OpenAPI Import Failures", ("The importer accepts a Swagger UI page or direct Swagger/OpenAPI document, resolves bounded same-origin references, previews operations and review counts, and changes the project only after explicit selection."), bullets=("Separate URL discovery failures from definition parsing and external-reference failures.", "Verify relative definition and reference URLs against the owning document URL.", "Expect explicit failures for cross-origin, circular, malformed, unreachable, excessively deep, or excessive-document reference graphs.", "Check Swagger 2.0 versus OpenAPI 3.x server/base-path construction.", "Confirm PATCH, HEAD, OPTIONS, JSON, URL-encoded forms, and text-only multipart forms map correctly.", "Confirm examples do not import bearer tokens, passwords, API keys, cookies, or credentials.", "Test Select All, Clear, partial selection, Add Selected, Add and Save Selected, save cancellation, persistence/reopen, and duplicate ID generation.")),
        SectionSpec("Persistence, Recovery, and Concurrent Saves", ("Project persistence validates the schema, redacts secret-bearing values, protects concurrent saves, creates recovery backups, and uses native atomic replacement where available."), bullets=("Do not bypass schema validation to open a malformed project.", "Preserve the original error and provide recovery guidance.", "Inspect project and backup paths without printing content that may be sensitive.", "Simulate a second save and verify it fails explicitly instead of racing.", "Verify a restored backup reopens and remains valid.")),
        SectionSpec("Diagnostics and Secret Safety", ("Treat logs, screenshots, saved responses, project exports, test fixtures, and diagnostics bundles as possible exfiltration paths."), bullets=("Use the shared redaction utility; do not invent local masking logic.", "Test bearer tokens, passwords, API keys, basic auth, cookies, client secrets, custom auth headers, and secret variables.", "Inspect the generated JSON bundle before sharing it.", "Never add a real credential to a unit test, snapshot, command, issue, or commit.")),
        ...
        SectionSpec("Definition of Done", bullets=("The requested behavior and failure modes have tests.", "Strict types cover parameters and return values.", "Errors are explicit and actionable.", "No secrets are committed or displayed.", "Representative tests, edge cases, coverage, lint, type checks, builds, and security checks pass.", "Relay Studio is running, the changed behavior is exercised interactively, and visible output plus runtime logs match expectations.", "Documentation and UML are updated when behavior or architecture changes.", "Commit messages include dev: jhaynes.")),
    )


def architecture_sections() -> tuple[SectionSpec, ...]:
    return (
        SectionSpec("Executive Overview", ("Relay Studio is an installable local desktop workbench for developers who need to construct, import, execute, organize, and debug REST APIs without sending project data to a hosted workspace. Its differentiators are selective Swagger/OpenAPI import, visual flow composition, strong secret redaction, local project recovery, and desktop-native execution.", "The product is intentionally an unsigned personal developer tool. Paid Apple and Microsoft readiness programs, app stores, hosted collaboration, and enterprise deployment certification are outside the current goal.")),
        SectionSpec("Product Capabilities", bullets=("Create, rename, delete, save, reopen, import, and recover local projects.", "Design REST services with GET, POST, PUT, PATCH, DELETE, HEAD, and OPTIONS plus parameters, headers, JSON/text/form bodies, auth, timeout, retry, proxy, and environment variables.", "Import a Swagger UI page or direct OpenAPI/Swagger definition, resolve bounded same-origin external references, review counts, and explicitly select operations.", "Run and cancel individual requests, inspect formatted responses, and export redacted diagnostics with a request inventory.", "Compose visual flows with dependency edges, failure branches, variable mappings, cleanup, and cancellation.", "Save redacted response artifacts, reopen them, and compare two artifacts for metadata plus structured JSON or raw-line differences.", "Package unsigned developer betas for macOS, Windows, and Linux.")),
        SectionSpec("System Context", ("The desktop process hosts a React webview and a Rust Tauri layer. The user controls local project/backup/response files and configured REST targets. GitHub Actions consumes dependency metadata and protected live-test configuration only for quality, security, and packaging automation."), bullets=("Trust boundary 1: React/TypeScript to registered Tauri commands.", "Trust boundary 2: native process to filesystem and configured HTTP(S) endpoints.", "Trust boundary 3: CI to npm, crates.io, RustSec, GitHub artifacts, and protected secrets.", "Trust boundary 4: imported API definitions and project files as untrusted input.")),
        SectionSpec("Frontend Architecture", ("App.tsx owns workbench composition and delegates domain behavior to typed modules. The shell includes native/shared commands, explorer, tabs, contextual tools, editors, inspector, response dock, and console. React Flow renders flow nodes and edges while service modules own graph validation and execution semantics."), bullets=("Keep presentation state close to the owning surface.", "Keep reusable domain behavior in src/services or src/project.", "Use shared typed errors and redaction utilities.", "Avoid global mutable state and UI-only business rules.")),
        SectionSpec("Native Architecture", ("The Rust layer provides the desktop capabilities that cannot be trusted to browser-only adapters: atomic project IO, backups, response files, dialogs, menus, Help resources, close protection, and native HTTP execution."), bullets=("Every command is explicitly registered.", "Capabilities remain least-privilege and are re-reviewed when commands or filesystem/network scope changes.", "Native errors cross the boundary as actionable failures.", "Packaged resources must be verified inside the installer, not only in development.")),
        SectionSpec("Project Data Model", ("A Relay project has an explicit format identifier and schema version. It owns services, environments, flows, saved-response metadata, import sources, and settings. Services own request construction and auth; flows reference services by ID and add graph/mapping state."), bullets=("Schema validation occurs before project data enters the application.", "Persistence and export redact sensitive values.", "Unknown or invalid schema shapes produce recovery guidance.", "Concurrent saves and partial native writes are guarded.")),
...
def sprint_sections() -> tuple[SectionSpec, ...]:
    ...
    sprint_summaries = (
        ...
        ("Sprint 16", "Implemented bounded same-origin external references, safe schema examples, PATCH/HEAD/OPTIONS, URL-encoded and text-only multipart forms, saved-response comparison, import review metrics, request-inventory diagnostics, and a live Body-panel layout fix."),
    )
...
def test_sections() -> tuple[SectionSpec, ...]:
    return (
        ...
        SectionSpec("Desktop Launch and Shell", steps=("Start Relay Studio in native development mode.", "Verify the activity rail, explorer, tabs, request composer, inspector, response dock, console, status, and native menus.", "Open the command palette and execute representative project/view commands.", "Resize through small, medium, and large desktop widths and verify no inaccessible controls or overlap.", "Inspect the webview console and Rust terminal for unhandled errors.")),
        SectionSpec("Project Lifecycle", bullets=("Create a project, edit it, observe dirty state, save, close, and reopen.", "Exercise Save As, Recent Projects, missing recent files, and unsaved-close prompts.", "Open invalid schema data and verify actionable recovery guidance.", "Simulate interrupted/native write behavior and restore the recovery backup.", "Trigger concurrent saves and verify the second operation fails explicitly.")),
        SectionSpec("Service Design and Execution", bullets=("Edit method, URL, headers, query/path parameters, body, auth, timeout, retry, proxy, and environment values.", "Run a successful request and inspect status, timing, content type, size, formatted body, console, and diagnostics.", "Exercise validation error, DNS/connection failure, timeout, malformed JSON, retryable failure, terminal HTTP response, and response-format fallback.", "Cancel a controlled delayed request and verify no retry continues.", "Rename and delete explorer items through their context menu.")),
        SectionSpec("Selective OpenAPI Import", steps=("Choose File > Import API Definition.", "Enter a direct OpenAPI/Swagger URL and a Swagger UI page URL in separate cases.", "Verify definition discovery, preview metadata, operation count, external-document count, form-body count, and deprecated count.", "Use Select All and Clear, then choose a strict subset of operations.", "Use Add Selected to review before saving, then repeat with Add and Save Selected and confirm the project save dialog opens.", "Save and reopen the `.restproj` file; confirm only selected services persist. Cancel a second save and confirm imported services remain dirty and editable.", "Inspect generated PATCH/HEAD/OPTIONS methods, path and parameters, safe JSON examples, URL-encoded or text-only multipart fields, folders, auth placeholders, IDs, dirty state, and source metadata.", "Repeat invalid URL/document, zero-selection, duplicate-operation, credential-like example, cross-origin, circular, malformed, unreachable, over-depth, and excessive-document cases.")),
...
def security_sections() -> tuple[SectionSpec, ...]:
    return (
        ...
        SectionSpec("Secret Classification and Redaction", bullets=("Bearer and OAuth tokens, passwords, API keys, cookies, client secrets, custom auth headers, and secret-marked variables are sensitive.", "Redact console events, diagnostics, project export/persistence, saved responses, screenshots/fixtures, errors, and snapshots.", "Preserve useful names and structure while replacing the value consistently.", "Never log a secret before redaction and never use real credentials in tests.")),
        SectionSpec("Tauri Boundary", bullets=("Restrictive CSP with object execution disabled and explicit script/connect sources.", "Reviewed capability file and registered commands only.", "Atomic writes and backups for project persistence.", "Native HTTP limited to user-configured targets and explicit errors.", "Re-review when adding commands, plugins, filesystem scope, remote content, updater behavior, or shell execution.")),
        ...
        SectionSpec("Known Risks and Deferrals", bullets=("Unsigned packages can trigger platform trust warnings.", "External OpenAPI references are intentionally same-origin and bounded; cross-origin graphs require the API owner to consolidate or proxy definitions locally.", "Multipart support is limited to text fields; file-path and binary-part upload remain deferred.", "User-configured endpoints may return hostile or very large data; response limits and formatting must remain bounded.", "Local project files are only as protected as the user's filesystem account.", "There is no hosted collaboration, public updater, store distribution, or enterprise deployment certification.")),
...
def main() -> None:
    ...
    manuals_only = "--manuals-only" in sys.argv
    ...
    build_manual(
        ...
        onboarding_sections(),
        "Read sequentially during onboarding, then return to the subsystem playbooks while debugging. Commands and paths are verified against the repository at Sprint 16.",
    )
    build_manual(
