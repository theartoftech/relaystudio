...
RETAINED_MARKDOWN = {
    ...
    "documentation/reviews/sprint-18d/closure-report.md",
    "documentation/reviews/sprint-18e/closure-report.md",
    "documentation/reviews/sprint-19/closure-report.md",
}
...
def add_cover(doc: Document, title: str, subtitle: str, audience: str, version_line: str = "Version 1.9 | Sprint 19 | July 2026") -> None:
...
def build_manual(filename: str, title: str, subtitle: str, audience: str, sections: Sequence[SectionSpec], callout: str, version_line: str = "Version 1.9 | Sprint 19 | July 2026", page_break_after_contents: bool = False, contents_space_after: float = 4) -> Path:
    doc = Document()
...
def onboarding_sections() -> tuple[SectionSpec, ...]:
    return (
        ...
        SectionSpec("Persistence, Recovery, and Concurrent Saves", ("Project persistence deeply validates schema-v1 state, canonically redacts secret-bearing values, removes multipart file authority, protects concurrent saves, creates recovery backups, and uses native atomic replacement where available."), bullets=("Do not bypass nested schema validation to open malformed services, environments, flows, responses, imports, or settings.", "Older schema-v1 files may omit settings introduced later; verify those missing fields receive current typed defaults while explicitly malformed values still fail.", "Project save/open/backup restore are bounded at 4 MiB before parse; verify the boundary and actionable oversized-file recovery path.", "Preserve the exact failing field path and provide recovery guidance.", "Inspect project and backup paths without printing content that may be sensitive.", "Confirm query/path/form canaries, URL userinfo, sensitive query values, and credential-shaped flow captures cannot survive save/reload.", "Flow runtime captures are available only inside the active run and are not written back to persistable environments.", "Simulate a second save and verify it fails explicitly instead of racing.", "Verify a restored backup reopens and remains valid."), page_break_before=True),
...
def architecture_sections() -> tuple[SectionSpec, ...]:
    return (
        ...
        SectionSpec("Product Capabilities", bullets=("Create, rename, delete, save, reopen, import, and recover local projects.", "Design REST services with GET, POST, PUT, PATCH, DELETE, HEAD, and OPTIONS plus parameters, headers, JSON/text/form bodies, multipart text/file parts, auth, timeout, retry, proxy, and environment variables.", "Import a Swagger UI page or direct OpenAPI/Swagger definition, explicitly review secondary destinations, resolve bounded same-origin external references, review counts, and select operations.", "Generate copyable HTTP, cURL, C#, Java, jQuery, Node.js, PHP, Python, and Ruby examples from request or flow tabs without execution.", "Run and cancel individual requests, inspect formatted responses and final origin, and export redacted diagnostics with a request inventory.", "Create a new GET request from an HTTP(S) value in formatted JSON while copying authorization only within the source request's origin.", "Compose visual flows with dependency edges, failure branches, protected destination variables, mappings, cleanup, and cancellation.", "Save redacted response artifacts, reopen them, and compare two artifacts for metadata plus structured JSON or raw-line differences.", "Package unsigned developer betas for macOS, Windows, and Linux."), page_break_before=True),
        SectionSpec("REST Code Examples", ("Sprint 19 adds a read-only code-generation surface to request and flow tab context menus. Output is derived from the typed request model, remains deterministic and bounded, and uses safe placeholders for credentials and local files."), bullets=("Generators: raw HTTP, cURL, C#, Java, jQuery, Node.js, PHP, Python, and Ruby.", "Coverage: query/path/header encoding, JSON/text bodies, URL-encoded forms, multipart text/file metadata, and supported HTTP methods.", "Flow examples preserve dependency order and annotate success/failure prerequisites plus response mappings; Java and jQuery output capture mapped JSON response values and reuse them in later requests, while application-specific branching remains the developer's responsibility. Java mapping output identifies its Jackson Databind dependency and credential-mask substitution requirement, then rejects non-2xx, empty, malformed JSON, missing, null, or non-scalar mapping responses without echoing response bodies.", "Generation never sends a request, reads a local multipart file, persists output, or mutates project state.", "Invalid, unsupported, oversized, or secret-bearing inputs receive explicit redacted guidance; output is capped at 256 KiB and flows at 100 requests."), page_break_before=True),
        ...
        SectionSpec("Frontend Architecture", ("App.tsx owns workbench composition and delegates domain behavior to typed modules. The shell includes native/shared commands, explorer, tabs, contextual tools, editors, inspector, response dock, and console. React Flow renders flow nodes and edges while service modules own graph validation and execution semantics."), bullets=("Keep presentation state close to the owning surface.", "Keep reusable domain behavior in src/services or src/project.", "Hypermedia link tokenization and linked-GET construction live in a typed service; App.tsx owns the tab/project state transition.", "Copy authorization only after exact scheme, host, and effective-port equality; reject cross-origin and embedded-credential destinations explicitly.", "Use shared typed errors and redaction utilities.", "Avoid global mutable state and UI-only business rules."), page_break_before=True),
...
def sprint_sections() -> tuple[SectionSpec, ...]:
    ...
    sprint_summaries = (
        ...
        ("Sprint\u00a018D", "Closed six execution/resource findings with runtime-only flow captures, explicit success/failure predecessor semantics, bounded native/browser responses and project files, bounded OpenAPI graph expansion, and bounded saved-response comparison."),
        ("Sprint\u00a018E", "Completed delivery hardening: scoped protected CI configuration, pinned first-party artifact actions, expanded lockfile and generated-artifact secret evidence, hardened OOXML validation, and published the final readiness decision."),
        ("Sprint\u00a019", "Added Postman-style request and flow code examples for raw HTTP, cURL, C#, Java, jQuery, Node.js, PHP, Python, and Ruby, generated from the typed request model without executing requests, reading files, mutating projects, or exposing credentials. Java and jQuery flows capture mapped JSON values and reuse them in later requests."),
    )
    for name, summary in sprint_summaries:
        ...
        sections.append(
            SectionSpec(
                ...
                (summary,),
                bullets=bullets,
                page_break_before=name in {"Sprint 7 / 7A", "Sprint\u00a018C", "Sprint\u00a018D"},
            )
        )
...
def test_sections() -> tuple[SectionSpec, ...]:
    return (
        ...
        SectionSpec("Security and Dependency Gates", code=("npm audit --audit-level=high", "npm run check:licenses", "cargo deny --manifest-path src-tauri/Cargo.toml check", "npm run check:secrets", "npm run check:documentation"), bullets=("No high-risk dependency finding may be ignored silently.", "No unapproved dependency license.", "No committed credential or secret-bearing generated artifact.", "Secret scanner limitations are reported with supported/unsupported counts rather than presented as a clean scan.", "OOXML text parts and relationship parts are parsed and checked, including custom XML, headers, footers, and embedded package metadata.", "Tauri CSP, capabilities, and commands remain reviewed.")),
        SectionSpec("Documentation and Artifact Gates", code=("npm run check:documentation",), bullets=("Every Word/VSDX artifact has the expected structure and every XML/rels part is parsed.", "External OOXML relationships fail with the artifact and relationship part identified.", "Synthetic credential canaries in custom XML or relationship-bearing parts fail before release evidence is accepted.", "Unsupported binary and oversized installer content is reported as a limitation by the repository/artifact secret scanner."), page_break_before=True),
        ...
        SectionSpec("Hypermedia Response Links", steps=("Send an authorized request whose formatted JSON includes a same-origin HAL href with path and query parameters.", "Activate the HTTP(S) value by pointer and by keyboard Enter; confirm a new GET request becomes the active tab without being sent.", "Verify path, query, Accept header, empty body, folder, timeout/retry settings, and an independent copy of the source authorization profile.", "Activate the same URL again and verify unique request IDs and tabs.", "Repeat with plain or malformed text, URL-like property names, non-HTTP schemes, embedded URL credentials, duplicate query names, and a different scheme, host, or effective port.", "Confirm unsafe/unsupported values leave project services unchanged, present actionable guidance, and never move authorization across origins."), page_break_before=True),
        SectionSpec("Request And Flow Code Examples", steps=("Right-click an open request tab, choose Generate Code, and open each of HTTP, cURL, C#, Java, jQuery, Node.js, PHP, Python, and Ruby.", "Verify method, URL, encoded parameters, headers, auth placeholders, and JSON/text/form/multipart bodies match the typed request without sending it.", "For multipart, confirm text fields remain and every local path becomes a SELECT_FILE placeholder; no file is read.", "Switch languages in the popup, use Copy Code, deny clipboard access once, and confirm both success and actionable failure feedback.", "Right-click a flow tab, select Java and then jQuery, and verify requests are dependency ordered with unique step variables and branch prerequisites; confirm mapped JSON response values are stored in flowVariables and reused by later requests. For Java, confirm the Jackson Databind dependency, credential-mask runtime-substitution notice, and explicit non-2xx/empty/malformed-JSON/missing/null/non-scalar guards are present without response-body disclosure.", "Attempt invalid requests, cyclic flows, unsupported language identifiers, more than 100 flow requests, and output over 256 KiB; confirm typed redacted errors and recovery.", "After every case, confirm the project dirty indicator, request/flow status, response history, and runtime logs are unchanged."), page_break_before=True),
...
def security_sections() -> tuple[SectionSpec, ...]:
    return (
        ...
        SectionSpec("CI and Release Policy", ("PR and main Release Gates run deterministic quality and security validation. When live REST configuration is missing they report the omission without making ordinary CI permanently red. Beta-tag and manual Package Beta runs require the protected live configuration and complete acceptance before the platform matrix starts."), bullets=("Protected configuration is base64-decoded only into the runner temporary directory and is scoped to the materialization/preflight step; checkout, setup, install, and packaging steps do not inherit the secret.", "Do not print, artifact, or cache the decoded file.", "First-party checkout and artifact actions are pinned to immutable revisions; third-party tool actions remain explicit channel-pinned and are reviewed before upgrades.", "The repository scanner detects modern token formats and credentialed registry URLs in lockfiles, scans generated bundle directories, and reports unsupported binary/oversized content as explicit limitations.", "The documentation validator parses every XML and relationship part in Word/VSDX archives and rejects external relationships or secret-like canaries.", "Artifacts are unsigned developer betas retained for a bounded period.", "Signing/notarization deferral is intentional and visible in release notes.")),
        SectionSpec("Sprint 18E Delivery Hardening", ("Sprint 18E is complete. Delivery evidence now distinguishes a clean scan from unsupported binary content, keeps protected live-test configuration out of setup/install scope, pins first-party artifact actions, and validates every OOXML text and relationship part before release readiness is decided."), bullets=("No unresolved critical or high finding remains in the Sprint 18A remediation register.", "Lower-severity scanner limitations are explicit, bounded to binary/oversized artifacts, and retained as release evidence rather than suppressed.", "The final readiness decision is based on type, lint, coverage, component/browser, Rust, dependency/license, documentation-artifact, secret, live-REST, and packaged-platform evidence when configured."), page_break_before=True),
        SectionSpec("Code Generation Boundary", ("Sprint 19 code generation remains entirely inside the React/TypeScript application boundary. It validates and normalizes the typed request model but never invokes browser Fetch, a Tauri HTTP command, native file IO, persistence, or response-history mutation."), bullets=("Credential-bearing environment variables, auth profiles, headers, query values, URL userinfo, JSON/form keys, and known literal canaries become placeholders or redaction masks before rendering.", "Multipart files are represented by SELECT_FILE placeholders; configured paths and file contents never enter generated output.", "A second credential-canary check rejects output if a known sensitive literal survives normalization.", "Generated output is capped at 256 KiB and flow expansion at 100 requests.", "Flow branch and mapping comments are guidance only; they do not silently claim application-specific orchestration semantics."), page_break_before=True),
        SectionSpec("Platform Packaging", bullets=("macOS: separate ARM64 (Apple Silicon) and x86_64 (Intel) DMGs with architecture-specific artifact names and post-build Mach-O verification, plus native menus, file dialogs, close protection, offline Help, and unsigned warning expectations.", "Windows: NSIS and MSI, title/caption behavior, WebView2, high contrast, multi-monitor, native paths, and unsigned warning expectations.", "Linux: DEB and AppImage, WebKitGTK dependencies, native paths, and permission handling.", "Every packaging change requires installer reconstruction and packaged-app smoke testing.")),
...
def main() -> None:
    ...
    if portfolio_only:
        build_manual(
            ...
            "Version 1.9 | Sprint 19 | July 2026",
            True,
            2,
        )
        return
    ...
    build_manual(
        ...
        onboarding_sections(),
        "Read sequentially during onboarding, then return to the subsystem playbooks while debugging. Commands and paths are verified against the repository through Sprint 19.",
    )
    build_manual(
