...
def build_manual(filename: str, title: str, subtitle: str, audience: str, sections: Sequence[SectionSpec], callout: str, version_line: str = "Version 1.8 | Sprint 18E | July 2026", page_break_after_contents: bool = False, contents_space_after: float = 4) -> Path:
    doc = Document()
...
def architecture_sections() -> tuple[SectionSpec, ...]:
    return (
        ...
        SectionSpec("REST Code Examples Roadmap", ("Sprint 19 is planned to add a read-only code-generation surface for the selected request. Output will be derived from the typed request model, remain deterministic and bounded, and use safe placeholders for credentials and local files."), bullets=("Planned generators: raw HTTP, cURL, C#, jQuery, Node.js, PHP, Python, and Ruby.", "Planned coverage: query/path/header encoding, JSON/text bodies, URL-encoded forms, multipart text/file metadata, and representative HTTP methods.", "Generation never sends a request, reads a local multipart file, or mutates project state.", "Invalid, unsupported, oversized, or secret-bearing inputs receive explicit redacted guidance."), page_break_before=True),
...
def sprint_sections() -> tuple[SectionSpec, ...]:
    ...
    sprint_summaries = (
        ...
        ("Sprint\u00a019 (planned)", "Add Postman-style REST code examples for raw HTTP, cURL, C#, jQuery, Node.js, PHP, Python, and Ruby, generated from the typed request model without executing requests or exposing credentials."),
    )
...
def main() -> None:
    ...
    if portfolio_only:
        build_manual(
            ...
            "Version 1.8 | Sprint 18E | July 2026",
            True,
            2,
        )
        return
    ...
    build_manual(
        ...
        onboarding_sections(),
        "Read sequentially during onboarding, then return to the subsystem playbooks while debugging. Commands and paths are verified against the repository through Sprint 18E.",
    )
    build_manual(
