...
| Persistence and redaction | Pass after Sprint 18C hardening | Full nested schema-v1 project state is validated with field paths before use. Shared classification covers API-key spelling variants, URL userinfo/query values, form/parameter rows, saved artifacts, diagnostics, comparisons, flow captures, and normalized errors before output is declared redacted. |
