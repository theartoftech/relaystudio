...
The library reflects completed Sprint 17 native multipart file workflows, completed Sprints 18A-18E review/remediation work, completed Sprint 19 request/flow code examples, and same-origin hypermedia navigation from formatted JSON responses. Relay Studio now generates bounded, copyable HTTP, cURL, C#, Java, jQuery, Node.js, PHP, Python, and Ruby examples from request or flow tabs without executing requests, reading multipart files, mutating projects, or displaying credential values. Java and jQuery flow output captures mapped JSON response values and reuses them in later requests. Java output identifies exported credential masks as required runtime substitutions and fails actionably before mapping reuse when a response is non-successful, empty, malformed JSON, missing, null, or non-scalar. Formatted JSON HTTP(S) values can create new same-origin GET request tabs with an independent copy of the source authorization profile; cross-origin, credential-bearing, and unsupported destinations fail before credentials can move. Relay Studio also keeps redirects on the reviewed origin, requires review before retrieving a Swagger UI secondary destination, bounds imported and compared documents, enforces response/project resource limits, evaluates flow branches by predecessor outcome, removes persisted multipart file authority, binds file approval to the current session and destination origin, rejects disguised response files, deeply validates project state, applies canonical redaction across persistence and output boundaries, and distinguishes clean secret scans from explicit unsupported-artifact limitations.

The packaged application includes a searchable, fully offline **Relay Studio Help** workspace. Open it from the command palette or native Help menu to find task-oriented guidance for projects and saving, requests, authentication, JSON/form/multipart bodies, OpenAPI import, response mappings, first-list-result flows, debugging, saved-response comparison, code examples, recovery, diagnostics, troubleshooting, and keyboard navigation. The bundled source of truth for this in-product user guide is `src/help/relay-studio-help.json`. macOS beta packaging produces separate ARM64 (Apple Silicon) and x86_64 (Intel) DMGs with architecture-specific artifact names and verifies the compiled Mach-O architecture before upload.
